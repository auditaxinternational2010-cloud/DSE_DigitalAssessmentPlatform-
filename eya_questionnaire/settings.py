...
ALLOWED_HOSTS = config(
    'ALLOWED_HOSTS',
    default='localhost,127.0.0.1,dolphin-app-u6d77.ondigitalocean.app',
...
# Uses DATABASE_URL when provided (DigitalOcean),
# individual variables otherwise (local).
...
# Shared Redis in production (used by rate limiting).
# Falls back to per-process local memory when REDIS_URL is unset,
# so local development and the test suite work unchanged.
...
AUTH_PASSWORD_VALIDATORS = [
    ...
    {
        'NAME':
            'accounts.validators.ComplexPasswordValidator',
    },
]
...
EVIDENCE_MAX_FILE_BYTES = config(
    'EVIDENCE_MAX_FILE_BYTES',
    default=5 * 1024 * 1024,
    cast=int
)

EVIDENCE_MAX_FILES_PER_RESP = config(
    'EVIDENCE_MAX_FILES_PER_RESP',
    default=5,
    cast=int
)

# Per-organization document-library storage quota
# (was per-questionnaire).
EVIDENCE_MAX_QUOTA_BYTES = config(
    'EVIDENCE_MAX_QUOTA_BYTES',
...
# A full category submit posts several fields per criterion
# (score, notes, numeric sub-fields, file inputs).
# Raise the default 1000-field cap to avoid TooManyFieldsSent.
...
# (replaces deprecated DEFAULT_FILE_STORAGE / STATICFILES_STORAGE)

# Set DEFAULT_FILE_STORAGE env var in production
# to switch to S3/GCS without code changes.
...
# The test suite must NEVER touch real object storage.
# Per-test @override_settings(DEFAULT_FILE_STORAGE=...)
# does NOT work:
# that setting was deprecated in Django 4.2 and REMOVED in 5.1.
#
# Forcing it here means a new test that forgets the decorator
# is still safe.

# ('pytest' in sys.modules covers a future pytest runner,
# where sys.argv has no 'test'.)
...
# Only required when DEFAULT_FILE_STORAGE =
...
# IMPORTANT:
# AWS_S3_REGION_NAME must match the bucket's actual region.
# If it is empty or wrong, boto3 talks to us-east-1 and S3
# answers every request with a redirect to the real region.
...
# (bucket.s3.region.amazonaws.com) avoid a redirect hop versus
# the legacy path style and are the recommended default.
...
# Only required when using Google Cloud Storage.
...
# Public share-dashboard endpoints.
# Loose — this blunts token brute-forcing
...
# Required for HTTPS in Django 4.0+

CSRF_TRUSTED_ORIGINS = config(
    'CSRF_TRUSTED_ORIGINS',
    default=(
        'http://localhost:8000,'
        'https://dolphin-app-u6d77.ondigitalocean.app'
    ),
    cast=lambda v: [s.strip() for s in v.split(',')],
)
...
EMAIL_HOST = config(
    'EMAIL_HOST',
    default='mail.auditaxinternational.co.tz'
)
...
SITE_URL = config(
    'SITE_URL',
    default='https://dolphin-app-u6d77.ondigitalocean.app'
)
...
# When set, new-participation-request alerts go to these
# addresses INSTEAD of every admin-role user.
